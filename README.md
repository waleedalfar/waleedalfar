...
[![GitHub](https://img.shields.io/badge/GitHub-waleedalfar-171515?style=flat-square&logo=github&logoColor=white)](https://github.com/waleedalfar)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Waleed%20Alfar-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](#)
...
|---|---|---|
| [**Inference Engine**](https://github.com/waleedalfar/inf-engine) | GPU inference engine built from scratch — custom Triton/CUDA kernels, continuous batching with a fixed resource-slot pool, and roofline-guided optimization. **19× throughput** over baseline, memory cut from O(N²) → O(N) (130× at scale) | `Python` `PyTorch` `Triton` `CUDA` `Linux` |
| [**StormLeads**](https://apps.apple.com/app/stormlead/id6803794866) | Multi-tenant B2B SaaS field-sales platform (mobile + web + API) for roofing canvassing, with row-level-security data isolation, PostGIS geospatial lead scoring, and TCPA-compliant consent enforcement | `TypeScript` `React Native` `Next.js` `NestJS` `PostgreSQL` `Redis` `Docker` `GCP` `Supabase` `Stripe` |
...
**Tools & Infra** &nbsp; `Linux` · `Kubernetes` · `Docker` · `Git` · `CI/CD` · `Redis` · `GCP (Cloud Run, Pub/Sub)` · `AWS Bedrock` · `CUDA`
